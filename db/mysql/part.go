...
func (d *DB) GetPartByLink(id string) (*models.Part, error) {
	...
	// Generate a link for the image if there isn't already
	if res.ImageID.Valid && !res.ImageLink.Valid {
		imgID := int(res.ImageID.Int64)
		...
		res.ImageLink.String = l.Link
		res.ImageLink.Valid = true
	}
