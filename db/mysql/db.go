...
const (
	...
	linkFields    = "link, partID, auto_generated"
...
func (d *DB) SearchParts(search models.Search) ([]models.Part, error) {
	...
	out := []models.Part{}
	// Create links for all the parts that have none
	for _, p := range res {
		if !p.Link.Valid {
...
func (d *DB) CreateLink(link models.Link) (*models.Link, error) {
	...
	// Check if the targeted part does exist
	query := fmt.Sprintf("SELECT COUNT(*) FROM %s WHERE id = ?", partTableName)
	var num int
	if err := d.db.Get(&num, query, link.PartID); err != nil {
		return nil, fmt.Errorf("failed to check for part existence: %w", err)
	}
	if num == 0 {
		return nil, fmt.Errorf("the linked part does not exist")
	}

	query = fmt.Sprintf("INSERT INTO %s(%s) VALUES(?, ?, ?)", linkTableName, linkFields)
	if _, err := d.db.Exec(query, link.Link, link.PartID, link.AutoGenerated); err != nil {
